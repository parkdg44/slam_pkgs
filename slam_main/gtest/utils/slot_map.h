...
// extern void _onAssertionFailed(const char* expression, const char* srcFile, unsigned int
// srcLine); #define SLOT_MAP_ASSERT(expression) (void)((!!(expression)) ||
// (_onAssertionFailed(#expression, __FILE__, (unsigned int)(__LINE__)), 0))
...
bool operator==(const Allocator<T1, Alignment>& /*lhs*/,
                const Allocator<T2, Alignment>& /*rhs*/) noexcept {
  return true;
}
...
/*
Even though slot map keys are technically typeless (uint64_t), we artificially add a new type to get
...
struct slot_map_key64 {
  ...
  static inline slot_map_key64 clearTagAndUpdateVersion(slot_map_key64 key,
                                                        version_t version) noexcept {
    SLOT_MAP_ASSERT(version != kInvalidVersion);
...
struct slot_map_key32 {
  ...
  static inline slot_map_key32 clearTagAndUpdateVersion(slot_map_key32 key,
                                                        version_t version) noexcept {
    SLOT_MAP_ASSERT(version != kInvalidVersion);
    id_type ver = (static_cast<id_type>(version) << kVersionShift) & kVersionMask;
...
/*
  A slot map is a high-performance associative container with persistent unique keys to access
  stored values. Upon insertion, a key is returned that can be used to later access or remove the
  values. Insertion, removal, and access are all guaranteed to take O(1) time (best, worst, and
  average case) Great for storing collections of objects that need stable, safe references but have
  no clear ownership.

  The difference between a std::unordered_map and a slot map is that the slot map generates and
  returns the key when inserting a value. A key is always unique and will only refer to the value
  that was inserted.
...
*/
template <typename T, typename TKeyType = slot_map_key64<T>, size_t PAGESIZE = 4096,
          size_t MINFREEINDICES = 64>
class slot_map {
 public:
  using key = TKeyType;
  ...
  /*
      ...
      Page size in elements. When all page items(slots) are disabled we automatically release that
     page to save memory. The page-based allocator also helps with pointers' stability and prevents
     memory spikes on reallocation.

      4096 seems like a reasonable number of items per page.
      I.e., for simple uint32_t type, it will take 16Kb of RAM per page = good compromise between
     CPU cache utilization, allocation granularity, and memory overhead from deactivated slots.
  */
  ...
  /*
      ...
      When a slot is reused, its version is automatically incremented (to make existing keys
     invalid). But since we only use 16-bits for version numbers, there is a possibility that the
     version counter will wrap around, and a new item will get the same key as a removed item.

      Once the version counter overflows, we disable that slot so that no new keys are returned for
     this slot (this gives us a guarantee that there are no key collisions)

      To prevent version overflow from happening too often, we need to ensure that we don't reuse
     the same slot too often. So we do not reuse indices as long as their number is below a certain
     threshold.
  */
 ...
 private:
  ...
  struct Page {
    ...
    void allocate() {
      ...
      size_type metaSize = static_cast<size_t>(sizeof(Meta)) * kPageSize;
      size_type dataSize = static_cast<size_t>(sizeof(ValueStorage)) * kPageSize;
      size_type alignedDataSize = align(dataSize, static_cast<size_t>(alignof(Meta)));
      size_type alignment =
          std::max(static_cast<size_t>(alignof(Meta)), static_cast<size_t>(alignof(T)));
      ...
      /*
        ...
        Passing a size which is not an integral multiple of alignment or an alignment which is not
        valid or not supported by the implementation causes the function to fail and return a null
        pointer (C11, as published, specified undefined behavior in this case, this was corrected by
        DR 460)
      */
  ...
  index_t appendElement() {
    ...
    index_t index = static_cast<index_t>(
        getIndexFromAddr(PageAddr{static_cast<size_type>(pages.size()) - 1, elementIndex}));
    return index;
  }
  ...
  inline static constexpr bool isPow2(INTEGRAL_TYPE x) noexcept {
    static_assert(std::is_integral<INTEGRAL_TYPE>::value,
                  "isPow2 must be called on an integer type.");
    return (x & (x - 1)) == 0 && (x != 0);
  }
  ...
  const ValueStorage& getValueByAddr(PageAddr addr) const noexcept {
      ...
      if (otherPage.meta) {
        ...
        if constexpr (std::is_standard_layout<T>::value && std::is_trivially_copyable<T>::value) {
          ...
          for (size_type elementIndex = 0; elementIndex < p.numUsedElements; elementIndex++) {
            PageAddr addr;
            addr.page = static_cast<size_t>(pageIndex);
  ...
  void callDtors() {
    ...
    for (size_t pageIndex = 0; pageIndex < pages.size(); pageIndex++) {
      ...
      for (size_t elementIndex = 0; elementIndex < page.numUsedElements; elementIndex++) {
        PageAddr addr;
        addr.page = static_cast<size_t>(pageIndex);
        addr.index = static_cast<size_t>(elementIndex);
  ...
  EraseResult eraseImpl(key k) {
    ...
    if constexpr (VERSION_CHECK) {
      version_t version = key::toVersion(k);
      if (slotVersion != version || slotVersion == key::kInvalidVersion ||
  ...
  */
  ...
  /*
    ...
    Automatically increases version for all the removed elements (the same as calling "erase()" for
  ...
  */
  ...
  /*
    If key exists returns a const pointer to the value corresponding to the given key or returns
    null elsewere.
  */
  const T* get(key k) const noexcept { return getImpl(k); }

  /*
    If key exists returns a pointer to the value corresponding to the given key or returns null
    elsewere.
  */
  ...
  */
  ...
  /*
    Same erase strategy, but reports whether anything was erased (convenient for parity with other
    APIs).
  */
  bool erase_if_present(key k) {
    EraseResult r = eraseImpl<true>(k);
    return r != EraseResult::NotFound;
  }

  /*
    Removes element (if such key exists) from the slot map, returning the value at the key if the
  ...
  */
  ...
  /*
    Exchanges the content of the slot map by the content of another slot map object of the same
    type.
  */
  ...
  // copy constructor
  slot_map(const slot_map& other) : numItems(0), maxValidIndex(0) { copyFrom(other); }
  ...
  slot_map(slot_map&& other) noexcept
      : numItems(other.numItems), maxValidIndex(other.maxValidIndex) {
 ...
 public:
  ...
  class values_iterator_impl {
   ...
   public:
    explicit values_iterator_impl(slot_map_ptr _slotMap, size_t index) noexcept
   ...
   private:
    ...
    template <bool>
    friend class values_iterator_impl;
  };
  ...
  class kv_iterator_impl {
   public:
    ...
    using value_ptr = std::conditional_t<IsConst, const T*, T*>;
    using storage_ref = std::conditional_t<IsConst, const ValueStorage&, ValueStorage&>;
    using KeyValue = std::conditional_t<IsConst, std::pair<key, const reference<const T>>,
   ...
   public:
    explicit kv_iterator_impl(slot_map_ptr _slotMap, size_type index) noexcept
        : slotMap(_slotMap),
          currentIndex(index),
          tmpKv(key::invalid(),
                std::conditional_t<IsConst, reference<const T>, reference<T>>(nullptr)) {}
   ...
   private:
    slot_map_ptr slotMap;
    size_t currentIndex;
    // unfortunately we need this to make it look like standard STL iterator (operator* and
    // operator->)
  ...
  class items_impl {
   ...
   public:
    ...
    iterator_type end() const noexcept {
      return iterator_type(slotMap, slotMap->getMaxValidIndex() + static_cast<size_t>(1));
    }
  };
...
namespace std {
template <typename T>
struct hash<typename dod::slot_map_key64<T>> {
  size_t operator()(const typename dod::slot_map_key64<T>& key) const noexcept {
    return key.hash();
  }
};

template <typename T>
struct hash<typename dod::slot_map_key32<T>> {
  size_t operator()(const typename dod::slot_map_key32<T>& key) const noexcept {
    return key.hash();
  }
};
